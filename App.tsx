...
import { GestureData, TreeState } from './types';
...
function App() {
  ...
  useEffect(() => {
    if (!process.env.API_KEY) {
      ...
      // We do not return early here to ensure the rest of the UI (including the manual button) remains functional
    }
        ...
        if (videoRef.current) {
          ...
          videoRef.current.onloadedmetadata = () => {
            ...
            geminiService.connect(videoRef.current!, (data) => {
              setGestureData(prev => {
                // NEW LOGIC: 
                // We always accept hand position (X, Y) for camera control.
                // But we ONLY accept state change if it is CHAOS (Unleash).
                // To restore to FORMED, the user must use the button.
